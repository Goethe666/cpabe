...
class MSP:
    ...
    def getCoefficients(self, tree):
        ...
        coeffs = {}
        self._getCoefficientsDict(tree, coeffs, self.group.init(ZR, 1))
        return coeffs
    ...
    def recoverCoefficients(self, list):
        ...
        for i in list2:
            ...
            for j in list2:
                if not (i == j):
                    result *= (self.group.init(ZR, 0) - j) / (i - j)
            coeff[int(i)] = result
        return coeff
    ...
    def _getCoefficientsDict(self, tree, coeff_list, coeff=1):
        ...
        if tree:
            ...
            if (node == OpType.AND):
                # FIX: convert_policy_to_msp uses additive sharing for AND gates.
                # Therefore, reconstruction coefficients are both 1.
                self._getCoefficientsDict(tree.getLeft(), coeff_list, coeff)
                self._getCoefficientsDict(tree.getRight(), coeff_list, coeff)
            elif (node == OpType.OR):
